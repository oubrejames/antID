...
def test_thresholds(model, test_loader, device, path_to_folder):
    ...
    while tested_count < len(test_loader):
        for anchor, positive, negative, label in test_loader:
            ...
            total_neg_dist += anchor_negative_dist
            total_pos_dist += anchor_positive_dist
    ...
    average_pos_dist = total_pos_dist/(tested_count+1)
    average_neg_dist = total_neg_dist/(tested_count+1)
    ...
    return best_threshold, average_thresh
