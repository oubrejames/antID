...
labelled_image_dir = "../unseen_body_passes/"
...
class SinglePassDetector:
    ...
    def __init__(self, patience=5):
        ...
        self.frames_without_count = 0
        self.start_count = False

    def update(self, ant_in_frame):
        """
        """
        if not ant_in_frame and self.start_count:
            self.frames_without_count += 1
            print("Have not seen an ant in {} frames".format(self.frames_without_count))
        
        if ant_in_frame:
            # print("First ant seen. Starting frames_without_count")
            self.start_count = True

    def detect_single_pass(self):
        """
        """
        
        if self.frames_without_count > self.patience:
            self.frames_without_count = 0
            self.start_count = False
...
def main():
    ...
    for ant_video in os.listdir(videos_directory):
        ...
        path_to_antid = labelled_image_dir + ant_id

        # Check if directory already exists
        if os.path.exists(path_to_antid):
            print("Images for " + ant_id + " already exist. Skipping...")
            continue

        os.mkdir(path_to_antid)

        # Open video with OpenCV
        cap= cv2.VideoCapture(path_to_video)

        # Create single pass through detection object to discern when ant makes one walk through
        single_pass = SinglePassDetector()
        pass_thru_counter = 0
        path_to_single_pass = path_to_antid + "/pass_" + str(pass_thru_counter)
        os.mkdir(path_to_single_pass)
        ...
        while(1):
            ...
            # Check if a single pass is done
            if single_pass.detect_single_pass():
                print("Making new pass through folder")
                pass_thru_counter += 1
                path_to_single_pass = path_to_antid + "/pass_" + str(pass_thru_counter)
                os.mkdir(path_to_single_pass)

            if bbox is None:
                single_pass.update(ant_in_frame=False)
                continue
            # print("Ant seen")
            single_pass.update(ant_in_frame=True)
            ...
            # Get ant label
            path_to_label = path_to_single_pass + "/im" + str(img_cnt) + ".jpg"
