...
bool detect_full_body(std::vector<cv::Point> points, cv::Mat* frame){
    ...
    if (points.size() > 0){
        ...
        // Filter if bounding box area is less than 26%  and greater than 70% of image area
...
int main(){
    ...
    {   
        ...
        while(1){
            ...
            if (detect_full_body(white_points, &frame)){
                // Save frame
                cv::imwrite(dir_labeled_imgs + "/ant_" + std::to_string(ant_id) + "_im_" + std::to_string(img_count) + ".jpg", frame);
                img_count++;
            }
    ...
    std::cout << "Number of images saved: " << img_count << std::endl << std::endl;
